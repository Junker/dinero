...
void fill_grid(void)
{
	...
	GdaDataModel *db_model = db_exec_select_sql ("SELECT a.id, b.date, strftime('%d.%m.%Y',date(b.date+1721425)), b.account_id, c. account_id, b.amount, b.currency_id,b.description \
	                                               FROM transfer a, operation b, operation c WHERE a.expenditure_id=b.id AND a.income_id=c.id \
	...
	ex_grid_lookup_field(grid, OUT_ACCOUNT_ID_COL, account_model, 0);
	refresh_account_model (); // Dirty hack
	ex_grid_lookup_field(grid, IN_ACCOUNT_ID_COL, account_model, 0);
	...
	ex_grid_column_set_title(grid, OUT_ACCOUNT_ID_COL, _("From"));
	ex_grid_column_set_title(grid, IN_ACCOUNT_ID_COL, _("To"));
...
void on_button_del_clicked (GtkButton *button,gpointer user_data)
{
	...
	guint id = ex_grid_get_selected_row_value(grid, 0);	
	
	gchar *sql = "DELETE FROM transfer WHERE id=##id::gint"; 
